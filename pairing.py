...
def performUpdate(sqlText, params):
    conn = psycopg2.connect(host='localhost',database=os.environ['DBNAME'],user=os.environ['USERNAME'],password=os.environ['PASSWORD'])
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor);
...
def pairing():
    ...
    if request.form['token'] == verification_token:
        ...
        if len(text) == 0:
            ...
            rows = performQuery("""SELECT * FROM availability 
            ...
            performUpdate("""INSERT INTO availability (channel_id,channel_name,person_id,person_name) 
                                    VALUES (%s,%s,%s,%s) 
            ...
            performUpdate("""UPDATE availability SET still_available = FALSE 
            ...
            payload = {'response_type': 'ephemeral','text':'unknown option: ' + text + user_name}
...
def pairingWith():
    ...
    if 'text' in request.form and len(request.form['text']) > 0:
        ...
        if pattern.match(request.form['text']) != None: # make sure that what we got passed was a username
            performUpdate("INSERT INTO pairings (person_id, pairs_with) VALUES(%s,%s)",(user_id,text));
...
def pairingEnd():
    ...
    performUpdate("UPDATE pairings SET ending = now() WHERE person_id = %s AND starting = ending;",(user_id,))
